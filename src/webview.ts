...
async function executeReqHandler(id: string, command: intf.Command): Promise<intf.Response> {
  ...
  } else if (command.prog === 'wc') {
    prog = config.get<string>('wcPath');
  } else if (command.prog === 'file') {
    prog = config.get<string>('fileUtilPath');
  } else {
  ...
  let result = await common.runProgramAsync(exec, timeout * 1000, command.env) as intf.Result;
  if (!result) {
